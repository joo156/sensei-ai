...
function Library() {
  ...
  function saveDraft() {
    ...
    if (draft.id) {
      ...
      toast.success(`Added to ${active.name}`);
    }
    setDraft(null);
  }
  ...
  return (
    <AppShell
      ...
      description={`Everything inside ${active.name}. Upload lecture files, write your own lecture notes, edit tags or remove material.`}
      actions={
    ...
    >
      <div
        ...
        onDrop={(e) => {
          ...
          toast.success(`Uploaded to ${active.name}`);
        }}
        className={cn(
      ...
      >
        ...
        <h2 className="text-lg font-semibold">Drop lecture material into {active.name}</h2>
          ...
          >
            <div className="surface-card mt-6 p-6">
              ...
              <div className="mt-4 flex gap-2">
                <Button onClick={saveDraft}>Save to {active.name}</Button>
      ...
      <div className="mt-8 flex flex-wrap items-center gap-3">
        ...
        <p className="text-muted-foreground text-sm">
          {filtered.length} items in {active.name}
        </p>
      </div>
